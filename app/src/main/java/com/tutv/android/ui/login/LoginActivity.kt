...
class LoginActivity : AppCompatActivity(), LoginView, View.OnClickListener {
    ...
    override fun setLoadingStatus(status: Boolean) {
        if (status) progressDialog = ProgressDialog.show(
                this,
                "Iniciando sesión...",
                "Espere mientras iniciamos sesión...")
        else progressDialog?.cancel()
    }

    override fun onClick(v: View) {
        when (v.id) {
            R.id.btlogin -> presenter?.loginBegin(editMail?.text.toString(), editPassword?.text.toString())
        }
    }

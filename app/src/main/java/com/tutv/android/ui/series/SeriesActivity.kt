...
class SeriesActivity : AppCompatActivity(), SeriesView {
    ...
    public override fun onCreate(savedInstanceState: Bundle?) {
        ...
        floatingActionButton.setOnClickListener(View.OnClickListener { event: View? -> seriesPresenter.onSeriesFollowClicked() })
        seasonRecyclerView = findViewById<View?>(R.id.series_season_recyclerview) as RecyclerView?
        seasonListAdapter = SeasonListAdapter(SeasonAndEpisodeClickedListener { s: Season?, e: Episode? -> seriesPresenter.onEpisodeClicked(s, e) })
        ...
        seriesPresenter = SeriesPresenter(this, seriesId, seriesRepository, schedulerProvider)
    }
    ...
    override fun showSeriesDescription(seriesDescription: String?) {
        seriesDescriptionTextView.setText(seriesDescription)
    }

    override fun showFollowerCount(followers: Int) {
        seriesFollowerCountTextView.setText("Followers: $followers")
    }

    override fun bindSeasons(seasonList: MutableList<Season?>?) {

...
class ProfileFragment : Fragment(), ProfileView {
    ...
    override fun onCreateView(inflater: LayoutInflater,
        ...
        val tv = TypedValue()
        if (context.getTheme().resolveAttribute(android.R.attr.actionBarSize, tv, true)) actionBarHeight = TypedValue.complexToDimensionPixelSize(tv.data, resources.displayMetrics)
        root.setPadding(0, statusBarHeight + actionBarHeight, 0, 0)
        profileImage = root.findViewById<ImageView?>(R.id.profile_image)
        username = root.findViewById<TextView?>(R.id.profile_username)
        mail = root.findViewById<TextView?>(R.id.profile_mail)
        logoutButton = root.findViewById<Button?>(R.id.profile_logout_button)
        logoutButton.setOnClickListener(View.OnClickListener { view: View? -> logoutButtonOnClick(view) })
        viewProfileInWebAppButton = root.findViewById<Button?>(R.id.profile_go_to_web_button)
        viewProfileInWebAppButton.setOnClickListener(View.OnClickListener { view: View? -> viewProfileInWebAppButtonOnClick(view) })
        loggedInLayout = root.findViewById(R.id.profile_logged_in_layout)
        loggedOutLayout = root.findViewById<LinearLayout?>(R.id.profile_not_logged_in_layout)
        signInButton = root.findViewById<Button?>(R.id.profile_sign_in_button)
        signInButton.setOnClickListener(View.OnClickListener { view: View? -> signInButtonOnClick(view) })
        progressBar = root.findViewById<ProgressBar?>(R.id.profile_progressbar)
        return root
    }

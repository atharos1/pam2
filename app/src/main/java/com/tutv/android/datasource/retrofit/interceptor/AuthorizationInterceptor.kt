...
class AuthorizationInterceptor(private val appContext: Context) : Interceptor {
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        invocation!!.method().getAnnotation(
            AuthenticatedRequest::class.java
        ) ?: return chain.proceed(chain.request())
        val requestBuilder = request.newBuilder()
        val sessionToken =
            appContext.getSharedPreferences("Token", Context.MODE_PRIVATE).getString("Token", "")
        if (sessionToken !== "") requestBuilder.addHeader("Authorization", sessionToken)
        val response = chain.proceed(requestBuilder.build())
        val avoidForceShowLoginScreenOnAuthFailAnnotation = invocation.method().getAnnotation(
            AvoidForceShowLoginScreenOnAuthFail::class.java
        )
